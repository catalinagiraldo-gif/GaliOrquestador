...
@Component({
  selector: 'app-impacto-ledger',
  standalone: true,
  imports: [CommonModule, RouterModule, GaliGlosarioDirective],
  template: `
    <div class="imp gali-stagger-in">
      <header class="imp__head">
        <a routerLink="/gali-v5-v2" class="imp__back">← Hoy</a>
        <h1 class="imp__title"><span aria-hidden="true">✦</span> El impacto de Gali</h1>
        <p class="imp__sub">Lo que Gali ha hecho por tu negocio. Tu relación con Gali compone valor cada semana.</p>
      </header>
      ...
      <section class="card">
        ...
        <div class="grid">
          <div class="stat stat--hero"><span class="stat__v">{{ s.pesos_ahorrados_label }}</span><span class="stat__l">ahorrados</span></div>
          <div class="stat"><span class="stat__v">{{ s.acciones_totales }}</span><span class="stat__l">acciones ejecutadas</span></div>
      ...
      <section class="acum">
        ...
        <div class="acum__stats">
          ...
          <span><strong>{{ a.novedades_evitadas }}</strong> novedades evitadas</span>
  ...
  `,
  styleUrl: './impacto-ledger.component.scss',
})
export class ImpactoLedgerComponent {
